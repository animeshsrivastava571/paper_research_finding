...
**Background.** Analysts increasingly work with AI assistants over financial documents — filings, credit files, transaction records — asking related questions across long sessions and revisiting the same companies. To stay within context limits, they compress conversation history and persist facts across sessions. Both ship as defaults in widely used memory libraries. In multi-agent deployments a retrieval agent gathers figures and an analysis agent consumes them via shared memory, compressing again at handoff. Compression is unavoidable at scale; its cost on financial text is unmeasured.

**Need.** A financial figure is meaningless without its qualifiers: the period it belongs to, the entity it describes, its scale, and the basis on which it was prepared. "Revenue was 233" is not a fact until one knows whose, when, and in what units. Default summarisation prompts never mention them; to a compressor, qualifiers read as redundancy. An unqualified figure can be literally correct and still mislead. An assistant may answer a current-year question with a prior year's figure, omit "millions" (a thousand-fold error), or surface one company's figures in another's. Recent work shows compression can change the investment judgment a source supports; where these errors originate is unknown.

**Aims.** The study has two parts. First, we quantify five memory-induced failure modes — context dilution, error propagation, temporal staleness, cross-entity contamination, and loss at inter-agent handoff — localise where along the pipeline qualifiers are lost, and establish which are most fragile. Second, we evaluate a remedy: qualifier-preserving summarisation instructions, structured fact schemas carrying period, entity, scale and basis, and entity-scoped storage. All are documented configuration parameters of existing libraries, not new software, adoptable without re-engineering.

**Methods.** We evaluate on ConvFinQA and FinQA: multi-turn analyst conversations over S&P 500 annual reports, 1999–2019. Company and reporting year are recoverable from record identifiers, so cross-period and cross-entity conditions arise naturally. Because conversations average under four turns — too short to trigger summarisation — we chain same-company conversations into sessions of fifteen to twenty. Identical question sets run through a two-agent pipeline — retrieval and analysis, sharing memory — under five conditions: no memory (floor); a widely used memory library at its shipped defaults; the same library with qualifier-preserving summarisation instructions; with structured fact schemas and entity-scoped storage; and full history without summarisation (ceiling). Correctness is verified numerically against executed reference answers, not by model judgement.

**Expected findings.** We expect qualifier retention to fall sharply once summarisation triggers and to degrade further each round and each handoff, with scale lost more readily than period. We expect entity-agnostic storage to produce cross-entity contamination, rising with sector similarity. We expect prompt-level fixes alone to prove insufficient — the summary budget is fixed and each round re-compresses the last — with structured storage needed to close the gap. Three deliverables follow: a loss profile across configurations and failure modes, as residuals against the ceiling; a turn threshold beyond which retention degrades, with guidance on which setting fixes which failure; and a released evaluation set of cross-period and cross-entity pairs.

---

*Word count (abstract body): 496 — 4 words under the limit*
