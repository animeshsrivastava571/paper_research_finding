...
**Expected findings.** We expect qualifier retention to fall sharply once summarisation triggers and to degrade further each round and each handoff, with scale lost more readily than period. We expect entity-agnostic default storage to produce cross-entity contamination, rising with sector similarity. We anticipate the remedy recovers a substantial but incomplete share of the ceiling. Three deliverables follow: a quantified loss profile across configurations and failure modes, as residuals against the ceiling; a turn threshold beyond which retention degrades, with guidance on which setting fixes which failure; and a released evaluation set of cross-period and cross-entity pairs.

---

*Word count (abstract body): 497 — 3 words under the limit*
